...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define "vagrant-as-dev"
  ...
  config.vm.provider "virtualbox" do |vb, override|
    ...
    if ENV.fetch('USE_NFS', false)
      override.vm.synced_folder ".", "/vagrant", type: "nfs",
        mount_options: ['actimeo=2', 'rw', 'tcp'],
        bsd__nfs_options: ['rw','no_subtree_check','all_squash','async'],
        fsnotify: true,
        exclude: ["node_modules", "log", "tmp", "output", "public"]
    else
      override.vm.synced_folder ".", "/vagrant", fsnotify: true
    end
  end
